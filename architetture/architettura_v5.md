flowchart TD
    ...
    subgraph B0["2. OFFLINE INGESTION PIPELINE"]
        ...
        B11["[NEW v5] CleanBase Pre-Index Gate<br/>anomaly detection over doc embeddings<br/>before entering the index;<br/>blocks poisoned/injected docs proactively<br/>(arXiv 2605.00460)"]
    end
    ...
    subgraph C0["3. SEMANTIC LAYER / ONTOLOGY PIPELINE"]
        ...
        C8["[NEW v5] EvoRAG KG Backpropagation<br/>feedback-driven KG update:<br/>retrieval failures → gradient-like signal →<br/>auto-refine KG triples without manual curation<br/>(arXiv 2604.15676)"]
        C9["[NEW v5] Order-Aware Hypergraph<br/>temporal ordering of knowledge:<br/>event sequences, version history, 'valid-at' facts<br/>hyperedges connect >2 temporally ordered nodes<br/>(arXiv 2604.12185)"]
    end
    ...
    subgraph F0["6. ONLINE QUERY PIPELINE"]
        ...
        F5["[v5] Retrieval Routing<br/>RouteRAG RL router: text / keyword / graph /<br/>tree / multimodal / thinking traces / multi-agent<br/>CacheRAG: semantic sub-plan cache →<br/>reuse retrieval plans for similar queries<br/>(RouteRAG: arXiv 2512.09487;<br/>CacheRAG: arXiv 2604.26176)"]
        F5B["[NEW v5] Semantic Plan Cache<br/>if similar query hit: return cached retrieval plan<br/>skip full retrieval pipeline<br/>(CacheRAG: arXiv 2604.26176)"]
    end
    ...
    subgraph G0["7. HYBRID RETRIEVAL AND RANKING"]
        ...
        G4["Graph Traversal<br/>(Bitemporal GraphDB + Order-Aware Hypergraph)"]

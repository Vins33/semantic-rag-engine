...
flowchart TD
    ...
    subgraph I0["9. GOVERNANCE AND FEEDBACK"]
        ...
        I10["[v7] Cost/Token Budget Controller<br/>runtime spending monitor per query/session:<br/>tracks prompt tokens + retrieval tokens + generation tokens<br/>enforces configurable per-query budget cap (e.g. 8k tokens)<br/>signals F6D to halt iterative retrieval on budget breach;<br/>downgrades H10 speculative gen to single-pass on breach;<br/>reports spend to I3 monitoring + I8 AutoRAGTuner for tuning"]
    end

    %% =========================================================
    %% MACRO AREA 10 - FEDERATED RAG LAYER [v7]
    %% Cross-organizational multi-silo RAG with privacy
    %% (FedRAG: arXiv 2603.25374)
    %% =========================================================

    subgraph J0["10. FEDERATED RAG LAYER [v7]"]
        J1["Silo Registry<br/>catalog of participating organizations;<br/>per-silo trust score, capability metadata,<br/>data residency constraints<br/>(FedRAG: arXiv 2603.25374)"]
        J2["[v7] Federated Query Planner<br/>decompose query → per-silo sub-queries<br/>assign sub-queries respecting data residency rules;<br/>no raw document crosses silo boundary<br/>(FedRAG: arXiv 2603.25374)"]
        J3["[v7] Silo-Local Retriever Interface<br/>each silo runs its own retrieval stack<br/>returns top-k ranked passages + confidence scores<br/>raw content never transmitted externally<br/>(FedRAG: arXiv 2603.25374)"]
        J4["[v7] Differential Privacy Encoder<br/>add calibrated Gaussian/Laplace noise to silo scores<br/>track privacy budget ε per silo per session<br/>enforce ε ≤ ε_max before aggregation<br/>(FedRAG: arXiv 2603.25374)"]
        J5["[v7] Privacy-Preserving Aggregator<br/>secure aggregation of noisy silo-level rankings<br/>cross-silo deduplication via locality-sensitive hashing<br/>RRF re-ranking over aggregated passages<br/>(FedRAG: arXiv 2603.25374)"]
    end
    ...
    %% Federated RAG path — triggered when cross-silo evidence needed
    F5 -- "Cross-silo query" --> J2
    ...
    %% Cost/Token Budget Controller connections [v7]
    I10 -.-> F2
    I10 -.-> F6D
    I10 -.-> H10A
    I10 -.-> H11A
    ...
    classDef governanceV7 fill:#8B0000,stroke:#0D0000,stroke-width:4px,color:#FFFFFF;
    classDef federated fill:#E8EAF6,stroke:#1A237E,stroke-width:4px,color:#0D1B2A;
    classDef federatedV7 fill:#3949AB,stroke:#0D1B6E,stroke-width:4px,color:#FFFFFF;
    ...
    class I10 governanceV7;
    class J1 federated;
    class J2,J3,J4,J5 federatedV7;
```
